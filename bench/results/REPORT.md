...
**Date:** 2026-05-15  
**Hardware:** 4x NVIDIA RTX 3090 (24 GB each), Vast.ai  
**vLLM:** 0.8.5.post1
...
| EPLB requires high imbalance | Confirmed: DeepSeek (1.5x) shows no gain; OLMoE (6.74x) shows large gains |
...
| EPLB sweet spot | **High imbalance (>3x) AND 40-80% GPU utilisation** |

---

## 1. Hook Overhead

**Method:** ABAB interleaved phases on vLLM TP=2. A phases run with profiling hooks active; B phases pause them via `PAUSE_FILE`. 150 requests per phase x 4 phases = 600 total. Welch t-test on the two TTFT distributions.
...
![Hook overhead chart](../charts/hook_overhead.png)
...
**Method:** vLLM TP=2, 2x RTX 3090. VLLM_EXPERT_MAP injected via environment variable, concurrency=8, 200 requests per phase.

| Metric | DEFAULT | EPLB | Delta |
|--------|---------|------|-------|
| P50 TTFT | 62.4 ms | 61.9 ms | -0.8% |
| P90 TTFT | 73.0 ms | 90.2 ms | +23.6% |
| Throughput | 119.5 req/s | 110.9 req/s | -7.2% |

**Result: no benefit.** With tensor parallelism, every GPU co-owns every expert's weight shards, so there is no per-GPU expert ownership and therefore no GPU-level load imbalance for EPLB to exploit. Expert parallelism (EP) is required.
...
**Method:** vLLM EP=4 (TP=1), 4x RTX 3090. Each GPU owns 16 of 64 experts exclusively. vLLM patched via `apply_eplb_patch.py` to honour `VLLM_EXPERT_MAP`. Activation snapshot: 100 prompts, top-8 routing, 1017 expert slots. Peak imbalance: **6.74x** on layer 0 (expert 6). EPLB plan: 772 moves, algorithm estimate 55.1%.

![Throughput sweep](../charts/throughput_sweep.png)
...
Even at moderate load, the 6.74x hot cluster on GPU 0 queues expert-heavy tokens. EPLB eliminates the bottleneck, with the largest improvement at the tail (P99: -77.9%).
...
At 8 in-flight requests per GPU, hot-expert GPUs are overloaded while cooler GPUs have spare capacity. EPLB moves the 6.74x hot cluster off the bottleneck GPU, eliminating the queue and freeing throughput.
...
At 16 in-flight per GPU every GPU is fully saturated, leaving no headroom to absorb redistributed experts. EPLB's moves force additional cross-GPU all-to-all routing without any compute relief; overhead wins.
...
**Method:** vLLM EP=4 (TP=4), 4x RTX 3090. Activation snapshot: 30 prompts, top-2 routing, 78 expert slots. Peak imbalance: **1.5x** (layers 1-3). EPLB algorithm estimate: 37.0% (conf 60%).
...
With 1.5x peak imbalance there is no meaningful expert hotspot to redistribute, and performance is flat across all metrics.
...
EPLB is actively harmful here. With a flat expert distribution, the remapping adds all-to-all routing hops with zero compute benefit. At c=16, GPU utilisation is high enough that the communication overhead cascades badly; p95 blows out to +226%.

---

## 5. Why DeepSeek-V2-Lite Is Already Balanced

The 1.5x peak imbalance on DeepSeek-V2-Lite (vs 6.74x on OLMoE) is not accidental; it reflects deliberate architectural choices:
...
**Top-2 vs top-8 routing.** With top-8 routing (OLMoE), a single preferred expert can absorb 8x its fair share of a token's attention. With top-2, the maximum contribution per token is much lower, so load can't compound as easily. Each token activates 12.5% of experts in OLMoE vs 3.1% in DeepSeek-V2-Lite.
...
**Condition 1 — Expert imbalance must be high (>3x recommended)**

| Model | Peak Imbalance | EPLB Outcome |
|-------|---------------|--------------|
| OLMoE-1B-7B | **6.74x** | Large gains at sweet-spot concurrency |
| DeepSeek-V2-Lite | **1.5x** | No gain; overhead dominates at all concurrencies |
...
- **40-80%:** Hot-expert GPUs are overloaded; cooler GPUs have headroom. EPLB redistributes load and the freed capacity absorbs it.
- **Above ~80%:** All GPUs are saturated, leaving no headroom to absorb moved experts; all-to-all communication cost dominates.

The crossover concurrency is hardware-specific (RTX 3090 saturates at c≈16-32 for a 1B model, c≈8-16 for a 16B model), but the utilisation window is consistent across GPU generations. 4xA100 or 8xH100 with NVLink would push the ceiling proportionally higher, through both greater compute capacity and cheaper all-to-all fabric.

**Note on algorithm accuracy:** The greedy EPLB planner estimated 37-55% TTFT improvement but measured gains on OLMoE were 7-57% depending on metric and concurrency, and 0% on DeepSeek. The planner models compute imbalance only, not accounting for cross-GPU communication overhead introduced by expert moves, nor for whether imbalance is high enough to warrant rebalancing. A topology-aware communication cost term and an imbalance threshold gate are the main accuracy improvements needed.
...
- **Imbalance threshold gate**: suppress EPLB plan when peak imbalance < 3x to avoid harmful rebalancing on balanced models.
...
Measured over 100 prompts x 32 generation steps, top-8 routing, 64 experts per layer.
...
Measured over 30 prompts x 32 generation steps, top-2 routing, 64 experts per layer.
