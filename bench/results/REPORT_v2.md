...
**Date:** May 2026  
**Hardware (v1 runs):** 4x NVIDIA RTX 3090 (24 GB each), Vast.ai  
**Hardware (hook overhead run):** 2x RTX 3090, TP=2 (same Vast.ai instance)  
...
**Method:** ABAB interleaved phases on vLLM TP=2, 2x RTX 3090. A phases run with profiling hooks active; B phases pause them. 150 requests per phase x 4 phases = 600 total. Welch t-test on the two TTFT distributions.
...
**Method:** vLLM 0.8.5.post1, EP=4 (one expert shard per GPU), 4x RTX 3090. Plumb profiles 100 prompts, runs `recommend_placement()` -> `_numa_finetune()`, produces an expert-to-GPU map. Map applied via `VLLM_EXPERT_MAP` and vLLM restarted. Benchmark runs against both configurations.

**Observed peak imbalance:** 6.74x on Layer 0 (Expert 6). Activation snapshot: 1017 expert slots, 772 moves in the EPLB plan.

![Throughput sweep](../charts/throughput_sweep.png)

### Concurrency = 8 (2 in-flight per GPU)

| Metric | Baseline | Plumb EPLB | Delta |
|---|---|---|---|
...
**Reading:** EPLB delivers large gains from c=8 through c=32, with the largest tail improvement at c=8 (P99 -77.9%) where the 6.74x hot expert cluster on GPU 0 creates a severe queue for expert-heavy tokens. At c=64 all GPUs are fully saturated; no spare capacity exists to absorb moved experts and the all-to-all routing overhead wins.
...
Plumb suppresses the placement recommendation entirely when peak imbalance is below 3x (`_LOW_IMBALANCE_THRESHOLD = 3.0`), with an explicit warning. This prevents harmful rebalancing on models that are already well-balanced by training design.

vLLM's native EPLB runs unconditionally. In v1 we benchmarked this on DeepSeek-V2-Lite (peak imbalance 1.5x) and measured P95 TTFT degrading by +226% at c=16 from blind rebalancing. vLLM's online EPLB has no equivalent gate.

### 4.3 No Redundant Expert Requirement

vLLM's online EPLB requires `--num-redundant-experts` to maintain additional copies of hot experts in GPU memory, making it unsuitable for memory-constrained deployments or setups where KV cache space is at a premium.
...
- vLLM 0.21.0, EP=2 (one expert shard per GPU), 2x RTX 4090
- OLMoE-1B-7B-0924
- Plumb profiling snapshot: 200 prompts
- ABAB interleave: 300 requests per phase x 4 phases
- Concurrency sweep: c=8, c=16, c=32
- Compared directly against a same-run baseline using the same version, hardware, and prompts
...
- **Same-version clean comparison (v3)**: EP=2, vLLM 0.21.0, 2x RTX 4090; script ready, deferred on cost
...
| Run | File | Notes |
|---|---|---|
| Hook overhead | `olmoe_1b7b_tp2_hook_toggle_20260514.json` | TP=2, 2x RTX 3090, vLLM 0.8.5 |
| TP=2 negative control | `olmoe_1b7b_eplb_concurrent_tp2_20260515.json` | Confirms EPLB requires EP not TP |
| Plumb EPLB EP=4 (c=8) | `olmoe_1b7b_eplb_ep4_20260515.json` | 4x RTX 3090, vLLM 0.8.5 |
